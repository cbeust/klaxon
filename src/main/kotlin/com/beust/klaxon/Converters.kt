...
fun matchType(prop: KProperty<*>?, value: JsonValue, type: Class<*>) : Boolean {
    val thisType = prop?.returnType ?: value.inside.javaClass
    val result =
        if (thisType is KType) {
            type.isAssignableFrom(thisType.javaType as Class<*>?)
        } else {
            thisType.javaClass.isAssignableFrom(type)
        }
    return result
}
